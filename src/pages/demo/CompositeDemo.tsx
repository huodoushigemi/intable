...
const [cols, setCols] = createSignal([
  ...
  { id: 'rate', name: 'Rate', width: 100, editable: true, type: 'rate', editorProps: { max: 5 }, zodSchema: z.coerce.number().min(1, 'Must be at least 1').max(5, 'Must be at most 5') },
  { id: 'progress', name: 'Progress', width: 120, editable: true, type: 'progress', zodSchema: z.coerce.number().min(0, 'Must be at least 0').max(100, 'Must be at most 100') },
  { id: 'hireDate', name: 'Hire Date', width: 110, editable: true, type: 'date', required: true },
  { id: 'email', name: 'Email', width: 180, editable: true, required: true, tooltip: true, zodSchema: z.string().email('Invalid email format') },
  { id: 'phone', name: 'Phone', width: 150, editable: true, type: 'tel', required: true, zodSchema: z.string().min(10, 'Phone must be at least 10 characters') },
...
function generateEmployee(i: number) {
  ...
  return {
    ...
    rate: Math.round((Math.random() * 4 + 1) * 2) / 2,
