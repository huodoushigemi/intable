...
import { type Plugin, Ctx } from '../index'
...
export const TreePlugin: Plugin = {
  ...
  rewriteProps: {
     ...
     */
    Td: ({ Td }, { store }) => o => {
      if (!store.props?.tree) return <Td {...o} />

      const { props } = useContext(Ctx)
      const firstCol = () => props.columns?.findIndex(e => !e[store.internal]) ?? -1
      if (firstCol() !== o.x) return <Td {...o} />

      const rowKey = store.props.rowKey
      const meta = () => store._treeMeta?.get(o.data?.[rowKey])
      const depth = () => meta()?.depth ?? 0
      const hasChildren = () => meta()?.hasChildren ?? false
      const isExpand = () => store.tree.has(o.data?.[rowKey])

      return (
        <Td {...combineProps(o, {
          get onDblClick() {
            return hasChildren() ? () => store.tree.toggle(o.data?.[rowKey]) : undefined
          },
        })}>
          <div class='flex items-center' style={`padding-left: ${depth() * 16}px`}>
            {hasChildren() ? (
              <ILucideChevronRight
                ...
                style={`transform: rotate(${isExpand() ? 90 : 0}deg); opacity: .6; flex-shrink: 0; transition: transform .15s`}
